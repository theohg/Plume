...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomServtestHdle;                    /**< serviceTest handle */
...
#define COPY_SERVICETEST_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x00,0x00,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for serviceTest +
   *                                2 for charWrite +
   *                              = 3
   *
  ...
  COPY_SERVICETEST_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomServtestHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: servTest, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: servTest \n\r");
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomServtestHdle,
                          UUID_TYPE_128, &uuid,
                          SizeCharwrite,
                          CHAR_PROP_WRITE,
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CHARWRITE:
      ret = aci_gatt_update_char_value(CustomContext.CustomServtestHdle,
                                       CustomContext.CustomCharwriteHdle,
                                       0, /* charValOffset */
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CHARWRITE:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomServtestHdle, CustomContext.CustomCharwriteHdle, SizeCharwrite, pPayload);
