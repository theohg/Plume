...
void EnterStop2Mode(void)
{
  ...
  // 2. Deinitialize peripherals instead of just stopping them
//  HAL_ADC_DeInit(&hadc1);
//  HAL_TIM_Base_DeInit(&htim2);
//  HAL_TIM_Base_DeInit(&htim16);
//  HAL_SPI_DeInit(&hspi2);  // Assuming hspi2 is used for WS2812 LEDs
//  HAL_UART_DeInit(&huart1); // Deinitialize UART after using it
//
//  // 3. Configure all GPIO pins for low power (analog mode)
//  HAL_PWREx_EnableGPIOPullDown(PWR_GPIO_A, 0xFFFF & ~(IMU_INT1_Pin)); // Keep IMU interrupt pin with pullup/down
...
static void Set_Inactivity_Delay(uint32_t delay_ms) {
    if (delay_ms < 1000) {
        printf("Inactivity delay must be at least 1000 ms. Setting to 1000 ms.\n");
        delay_ms = 1000;
    }
